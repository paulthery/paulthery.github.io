...
const CONFIG = {
  ...
  // Art direction opens on a random image. For the 'work' album the draw is
  // restricted to this shortlist, given as file numbers so it matches both the
  ...
  // Index bio typewriter
  TYPEWRITER_START_DELAY: 450, // waits for the panel slide-in
  TYPEWRITER_SPEED: 5, // ms per character
...
/**
 * Index of the image art direction opens on. The 'work' album draws from
 * CONFIG.WORK_INTRO_IMAGES; every other album keeps the plain random draw.
 * `files` holds one variant per image, so matching on the leading number
 * works on desktop and on mobile alike.
 */
function pickIntroIndex(files, category, albumKey) {
  if (!files || !files.length) return 0;

  if (category === 'artdirection' && String(albumKey).toLowerCase() === 'work') {
    const shortlist = [];
    files.forEach((file, index) => {
      const name = file.split('/').pop() || '';
      const number = (name.match(/^(\d+)_/) || [])[1];
      if (number && CONFIG.WORK_INTRO_IMAGES.includes(number)) {
...
// ========================================
// 📦 MODULE: INDEX TYPEWRITER
// ========================================

/**
 * Writes the index bio paragraphs character by character. The rest of the
 * panel (tabs, album list, footer) appears with the slide-in as before.
 * Every character is wrapped in its own span that stays in the flow with
 * `visibility: hidden`, so line breaks, justification and the bio centering
 * are already final while the text is being typed: nothing reflows.
 */
class IndexTypewriter {
  constructor(root) {
    this.root = root;
    this.rafId = null;
    this.caret = null;
  ...
  // Wrap each character of an element into its own hidden span
  static split(element) {
    if (!element) return [];
    if (element.dataset.twSplit === 'true') {
      return Array.from(element.querySelectorAll('.tw-char'));
    }
    const textNodes = [];
    ...
    textNodes.forEach(node => {
      // Whitespace between tags carries no glyph, leave it alone
    ...
    element.dataset.twSplit = 'true';
    return chars;
  }

  paragraphs() {
    return Array.from(this.root.querySelectorAll('.bio-section p'));
  }

  // Split the bio and hide it, before the panel slides in
  prepare() {
    if (!this.root) return;
    this.cancel();
    this.paragraphs().forEach(paragraph => IndexTypewriter.split(paragraph));
    this.root.querySelectorAll('.tw-char').forEach(char => {
      char.classList.remove('is-visible', 'tw-caret');
    });
  }
  ...
  type(startDelay = 0) {
    ...
    const steps = [];
    if (startDelay) steps.push({ span: null, delay: startDelay });
    // One continuous stream: the blank line between paragraphs costs nothing,
    // typing runs straight on to the next character.
    this.paragraphs().forEach(paragraph => {
      IndexTypewriter.split(paragraph).forEach(span => {
        steps.push({ span, delay: CONFIG.TYPEWRITER_SPEED });
      });
    });
    ...
    const tick = now => {
      ...
      let budget = 600;
      while (index < steps.length && elapsed >= steps[index].delay && budget > 0) {
        elapsed -= steps[index].delay;
        if (steps[index].span) this.reveal(steps[index].span);
        index += 1;
        budget -= 1;
      }
  ...
  reveal(span) {
    ...
    // Spaces do not move the caret: it stays on the last printed glyph
    if (!span.textContent.trim()) return;
    if (this.caret && this.caret !== span) this.caret.classList.remove('tw-caret');
    span.classList.add('tw-caret');
    this.caret = span;
  }

  clearCaret() {
    if (!this.caret) return;
    this.caret.classList.remove('tw-caret');
    this.caret = null;
  }

  // Print everything at once (click during the animation, reduced motion)
  complete() {
    if (!this.root) return;
    this.cancelFrame();
    this.root.querySelectorAll('.tw-char').forEach(char => char.classList.add('is-visible'));
    this.clearCaret();
  }
...
(function () {
  ...
  if (isStudio) {
    ...
    // Keep the path: this also runs on /photography, not just the root
    const studioUrl = 'https://paulthery.studio' + window.location.pathname;
    if (canonical) canonical.href = studioUrl;
    if (ogUrl) ogUrl.content = studioUrl;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // currentCategory, currentAlbumKey already declared globally
  ...
  function finishLoad(files) {
    ...
    if (currentCategory === 'artdirection' && isFirstArtDirectionLoad) {
      currentIndex = pickIntroIndex(images, currentCategory, currentAlbumKey);
  ...
  if (indexTrigger && indexOverlay && indexContent && imageWrapper) {
    const indexTypewriter = new IndexTypewriter(indexContent);
    // A click anywhere prints the rest of the panel immediately
    indexOverlay.addEventListener('pointerdown', () => indexTypewriter.complete(), {
      capture: true,
    });
    let openedCategory = null;
    // The bio is written out once per visit, on the first opening of the index.
    // Later openings show it already printed.
    let bioHasTyped = false;
    const openIndex = (category = lastActiveCategory, userInitiated = false) => {
      // On /artdirection and /photography this fires twice: once from
      // handleRouting when the intro ends, once from the auto-open timer.
      // Replaying it re-measures the panel mid-rebuild and shifts the bio,
      // so ignore a second open of the category already on screen.
      const alreadyOpen = indexOverlay.classList.contains('active') && !isClosing;
      if (alreadyOpen && openedCategory === category) return;
      ...
      // openIndex can fire twice on /artdirection and /photography: once from
      // handleRouting when the intro ends, once from the auto-open timer.
      // Only (re)write the bio when the panel was not already on screen.
      if (!indexOverlay.classList.contains('active') || isClosing) {
        if (!bioHasTyped) {
          // Hide the bio before the panel slides in, layout already final
          indexTypewriter.prepare();
          indexTypewriter.type(CONFIG.TYPEWRITER_START_DELAY);
          bioHasTyped = true;
        } else {
          // Already written earlier in this visit, and a close mid-typing can
          // have left it half printed: show it in full straight away.
          indexTypewriter.complete();
        }
      }
      ...
      Promise.all([manifestPromise, orderPromise])
        .then(([manifest, order]) => {
          ...
          function renderPhotography() {
            ...
            const renderedWork = unifiedList.children.length;
            ...
            // Skip the gap when either group is empty, it would dangle
            if (renderedWork && Object.keys(projectData).length) {
